...
export default function AdminDashboard() {
  ...
  const [assignCourseSuggestionsOpen, setAssignCourseSuggestionsOpen] = useState(false);
  ...
  const normalizeCourseName = useCallback((value = "") => String(value || "").toLowerCase().replace(/[^a-z0-9]+/g, " ").trim(), []);

  const activeCourses = useMemo(
    () => (courses || []).filter((course) => String(course.status || "active").toLowerCase() === "active"),
    [courses]
  );

  const selectedAssignCourse = useMemo(
    () => activeCourses.find((course) => String(course.id || "") === String(assignCourseId || "")),
    [activeCourses, assignCourseId]
  );

  const filteredAssignCourseSuggestions = useMemo(() => {
    const search = assignCourseSearch.trim().toLowerCase();
    const items = activeCourses.filter((course) => {
    ...
    items.sort((a, b) => {
      const aTitle = String(a.title || a.course_name || "");
      const bTitle = String(b.title || b.course_name || "");
      if (!search) return aTitle.localeCompare(bTitle);
      const aStarts = aTitle.toLowerCase().startsWith(search);
      const bStarts = bTitle.toLowerCase().startsWith(search);
      if (aStarts !== bStarts) return aStarts ? -1 : 1;
      return aTitle.localeCompare(bTitle);
    });

    return items;
  }, [activeCourses, assignCourseSearch]);

  const customAssignCourseSuggestion = useMemo(() => {
    const title = assignCourseSearch.trim();
    if (!title) return null;
    const normalized = normalizeCourseName(title);
    if (!normalized) return null;
    if (activeCourses.some((course) => normalizeCourseName(course.title || course.course_name) === normalized)) return null;
    return title;
  }, [activeCourses, assignCourseSearch, normalizeCourseName]);
  ...
  }, [adminRole, debouncedSearch, userRoleFilter, userStatusFilter, usersPage, usersPageSize, selectedUserId, users.length]);
  ...
  const loadAllocationStudents = useCallback(async ({ append = false, force = false, pageOverride = null } = {}) => {
    ...
    try {
      ...
      const cachedStudents = adminApi.getCachedStudents(fallbackQuery);
      const fallbackUsers = (Array.isArray(users) ? users : []).filter((item) => (item.role || "student") === "student");
      const fallbackItems = (cachedStudents?.items || fallbackUsers).map((item) => ({ ...item, role: item.role || "student" }));
      if (fallbackItems.length > 0) {
        const nextItems = append ? [...allocationStudents, ...fallbackItems] : fallbackItems;
        setAllocationStudents(dedupeUsersForView(nextItems));
        setAllocationStudentsTotal(cachedStudents?.total || nextItems.length);
        setAllocationStudentsHasMore(Boolean(cachedStudents?.hasMore));
        setStatus("Showing cached registered students while the backend is offline");
        return;
      }
  ...
  }, [assignCourseId, debouncedAllocationSearch, assignDepartment, assignBatch, assignYear, allocationStudentsPage, allocationStudents, users]);
  ...
  }, [canAccessSection, loadDashboardData, loadUsersOnly, loadCoursesFeed, loadAllocationStudents, loadAllottedSkills, assignCourseId, loadCommunicationTasksAndSubmissions]);
  ...
  }, [loadDashboardData, loadUsersOnly, clearAdminCaches]);
  ...
  useEffect(() => {
    ...
    setAssignCourseSearch(selectedAssignCourse?.title || selectedAssignCourse?.course_name || "");
  }, [assignCourseId, selectedAssignCourse]);
            ...
            <div className="space-y-3">
              <div className="relative">
                <input
                  ...
                  onFocus={() => setAssignCourseSuggestionsOpen(true)}
                  onChange={(e) => {
                    setAssignCourseSearch(e.target.value);
                    setAssignCourseId("");
                    setAssignCourseSuggestionsOpen(true);
                  }}
                  placeholder="Search or type course name"
                  className="w-full rounded-xl border border-slate-200 px-3 py-2"
                />
                {assignCourseSuggestionsOpen && (
                  <div className="absolute z-20 mt-2 w-full overflow-hidden rounded-xl border border-slate-200 bg-white shadow-lg">
                    <div className="max-h-56 overflow-y-auto overscroll-contain p-1">
                      {filteredAssignCourseSuggestions.length > 0 ? (
                        filteredAssignCourseSuggestions.map((course) => {
                          ...
                          return (
                            <button
                              ...
                              onClick={() => {
                                ...
                                setAssignCourseSearch(title);
                                setAssignCourseSuggestionsOpen(false);
                              }}
                            >
                              {title}
                            </button>
                      ...
                      {customAssignCourseSuggestion && (
                        <>
                          {filteredAssignCourseSuggestions.length > 0 && <div className="my-1 border-t border-slate-100" />}
                          <button
                            type="button"
                            className="w-full text-left px-3 py-2 text-sm rounded-lg bg-emerald-50 text-emerald-800 hover:bg-emerald-100 cursor-pointer"
                            onMouseDown={(event) => event.preventDefault()}
                            onClick={() => {
                              setCourseDraftTitle(customAssignCourseSuggestion);
                              setActiveSection("courses");
                              setAssignCourseSuggestionsOpen(false);
                              setAssignCourseSearch(customAssignCourseSuggestion);
                            }}
                          >
                            Create new course: {customAssignCourseSuggestion}
                            <span className="block text-xs text-emerald-700/80 mt-0.5">Open the Courses tab and enter the remaining details</span>
                          </button>
                        </>
                      )}
                    </div>
